...
// Helper function to get max icons based on screen width
function getMaxIcons() {
  return window.innerWidth <= 530 ? 7 : 10;
}
...
function createIconArray(container, count, type) {
  ...
  if (existingIcons < maxIcons) {
    for (let i = existingIcons; i < maxIcons; i++) {
      ...
      icon.className =
        type === "drive" ? "drive-icon icon-enter" : "money-icon icon-enter";
...
function animateNumber(
  ...
  const startValue =
    parseFloat(element.textContent.replace(/,/g, "").replace("K", "000")) || 0;
  ...
  function update(currentTime) {
    ...
    element.textContent = useKFormat
      ? formatNumberWithK(currentValue)
...
function updateBarChart(year1Value, year2Value, year3Value) {
  ...
  if (maxValue > 0) {
    ...
    const year1Height = Math.max(
      MIN_BAR_HEIGHT,
      (year1Value / maxValue) * MAX_BAR_HEIGHT
    );
    const year2Height = Math.max(
      MIN_BAR_HEIGHT,
      (year2Value / maxValue) * MAX_BAR_HEIGHT
    );
    const year3Height = Math.max(
      MIN_BAR_HEIGHT,
      (year3Value / maxValue) * MAX_BAR_HEIGHT
    );
...
function calculateAndUpdate() {
  ...
  const year2SaveValue = Math.max(
    0,
    annualOutsourcingCost * 2 - lsoftCostValue
  );
  const year3SaveValue = Math.max(
    0,
    annualOutsourcingCost * 3 - lsoftCostValue
  );
...
function setupDraggable(container, type) {
  ...
  const handleMove = (clientX) => {
    ...
    if (type === "drives") {
      currentDrives = Math.round(
        MIN_DRIVES + percentage * (MAX_DRIVES - MIN_DRIVES)
      );
